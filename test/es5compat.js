...
describe('Array', function() {
    var testSubject;
    beforeEach(function() {
        testSubject = array([2, 3, undefined, true, 'hej', null, false, 0]);
    });
    ...
    describe('forEach', function() {
        ...
        beforeEach(function() {
            expected = {0:2, 1: 3, 2: undefined, 3:true, 4: 'hej', 5:null, 6:false, 7:0 };
            actual = {};
        });
        ...
        it('should not affect elements added to the array after it has begun', function() {
            ...
            assert(deepEqual(arr.toArray(), [1,2,3,4,5,6]));
            assert(i === 3);
        });
        ...
        it('should set the right context when given none', function() {
            ...
            assert(context === function() {return this}.call());
        });
    ...
    describe('some', function() {
        ...
        it('should set the right context when given none', function() {
            var context;
            array([1]).some(function() {context = this;});
            assert(context === function() {return this}.call());
        });
        ...
        it('should return false if there are no elements', function() {
            actual = array([]).some(function() { return true; });
            assert(!actual);
        });
    ...
    describe('every', function() {
        ...
        it('should pass the correct values along to the callback', function() {
            ...
            var arr = array(['1']);
            arr.every(callback);
            assert(deepEqual(callback.lastCall.args, ['1', 0, arr]));
        });
        ...
        it('should set the right context when given none', function() {
            var context;
            array([1]).every(function() {context = this;});
            assert(context === function() {return this}.call());
        });
        
        it('should return true if the array is empty', function() {
            actual = array([]).every(function() { return true; });
            assert(actual);

            actual = array([]).every(function() { return false; });
            assert(actual);
        });

        it('should return true if it runs to the end', function() {
            actual = array([1,2,3]).every(function() { return true; });
            assert(actual);
        });
        
        it('should return false if it is stopped before the end', function() {
            actual = array([1,2,3]).every(function() { return false; });
            assert(!actual);
        });
    ...
    describe('indexOf', function() {
        ...
        beforeEach(function() {
            testSubject = array([2, 3, undefined, true, 'hej', null, 2, false, 0]);
        });

        it('should find the element', function() {
            assert(testSubject.indexOf('hej') === 4);
        });
        
        
        it('should not find the element', function() {
            assert(testSubject.indexOf('mus') === -1);
        });
        
        it('should find undefined as well', function() {
            assert(testSubject.indexOf(undefined) !== -1)
        });

        
        it('should skip unset indexes', function() {
            assert(testSubject.indexOf(undefined) === 2);
        });
        ...
        it('should skip the first if fromIndex is set', function() {
            assert(testSubject.indexOf(2, 2) === 6);
            assert(testSubject.indexOf(2, 0) === 0);
            assert(testSubject.indexOf(2, 6) === 6);
        });

        it('should work with negative fromIndex', function() {
            assert(testSubject.indexOf(2, -3) === 6);
            assert(testSubject.indexOf(2, -9) === 0);
        });
        
        /*
        it('should work with fromIndex being greater than the length', function() {
            assert(testSubject.indexOf(0, 20) === -1);
        });
